...
static int create_pipe_and_check(int *pipe_fd)
{
    if (pipe(pipe_fd) == -1)
    {
        perror("pipe");
...
static pid_t fork_left_child(int *pipe_fd, t_exec_node *node, t_shell *shell)
{
    pid_t left_pid;
...
static pid_t fork_right_child(int *pipe_fd, t_exec_node *node, t_shell *shell, pid_t left_pid)
{
    pid_t right_pid;
...
static void cleanup_argv_on_error(char **argv, int count)
{
    int j;
