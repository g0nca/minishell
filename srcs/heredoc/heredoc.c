...
/*   Updated: 2025/09/23 16:33:42 by ggomes-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    char    *line;
    int     delimiter_len;    delimiter_len = ft_strlen(delimiter);
...
}char    *create_heredoc(const char *delimiter, t_shell *shell)
{
    ...
    char    *filename;
    pid_t   pid;
    int     status;    fd = create_temp_file(&filename);
    if (fd < 0)
        return (NULL);
