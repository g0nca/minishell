...
/*   Updated: 2025/04/24 11:03:04 by joaomart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	process_heredoc(t_token *token, t_shell *shell)
{
	...
	while (current)
	{
		if (current->type == TOKEN_HERE_DOC && current->next)
		{
			...
			// Em vez de abrir e fazer dup2(), guardamos o caminho
			current->heredoc_path = filename;

			// Remover o delimitador, mas manter o '<<' para sabermos que tem heredoc
			remove_token(&token, delimiter_token);

			current = token; // reset da lista após modificação
			continue;
		}
